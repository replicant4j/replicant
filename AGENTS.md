...
- `.bazelbsp/` and `.idea/` are generated local IDE state; keep them untracked and outside repository-wide
  Buildifier checks.
...
- Bazel compiles real GWT 2.13.1 smoke outputs for Replicant, ReplicantDev, ReplicantDebug, and React4j. The GWT
  aspect under `tools/gwt` consumes both transitive source jars and annotation-processor generated source jars.
