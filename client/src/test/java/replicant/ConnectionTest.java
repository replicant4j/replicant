...
public class ConnectionTest extends AbstractReplicantTest {
    ...
    public void canGroupRequests() {
        ...
        for (final AreaOfInterestRequest r1 : requests) {
            for (final AreaOfInterestRequest r2 : requests) {
                final boolean expected = (r1 == r2 && null != r1.getAddress().rootId())
    ...
    public void canGroupRequests_presentInCache() {
        ...
        final AreaOfInterestRequest requestA =
                new AreaOfInterestRequest(addressA, AreaOfInterestRequest.Type.ADD, null);
        final AreaOfInterestRequest requestB =
                new AreaOfInterestRequest(addressB, AreaOfInterestRequest.Type.ADD, null);
    ...
    public void isAreaOfInterestRequestPending_passingNonnullFilterForDelete() {
        ...
        final IllegalStateException exception = expectThrows(
                IllegalStateException.class,
                () -> connection.isAreaOfInterestRequestPending(AreaOfInterestRequest.Type.REMOVE, address, filter));
        assertEquals(
                exception.getMessage(),
                "Replicant-0025: Connection.isAreaOfInterestRequestPending passed a REMOVE request for address '1.0'"
                        + " with a non-null filter 'MyFilter'.");
    }
    ...
    public void pendingAreaOfInterestRequestQueries_jumbledAggregate() {
        ...
        connection.injectCurrentAreaOfInterestRequest(
                new AreaOfInterestRequest(address1, AreaOfInterestRequest.Type.ADD, filter1));
        connection.injectCurrentAreaOfInterestRequest(
                new AreaOfInterestRequest(address4, AreaOfInterestRequest.Type.ADD, filter4));
