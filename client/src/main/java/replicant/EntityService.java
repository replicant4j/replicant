...
abstract class EntityService extends ReplicantService {
    // Entity map: Type => ID
    ...
    private Entity createEntity(
            @NonNull final Map<Integer, Entity> typeMap,
            @Nullable final String name,
            @NonNull final Class<?> type,
            final int id) {
        getEntitiesObservableValue().preReportChanged();
        final Entity entity = Entity.create(Replicant.areZonesEnabled() ? getReplicantContext() : null, name, type, id);
        DisposeNotifier.asDisposeNotifier(entity).addOnDisposeListener(this, () -> destroy(entity), true);
