 ...
 */
final class SpyImpl implements Spy {
    /**
    ...
    public void addSpyEventHandler(@NonNull final SpyEventHandler handler) {
        if (Replicant.shouldCheckApiInvariants()) {
            apiInvariant(
                    () -> !_spyEventHandlers.contains(handler),
    ...
    public void reportSpyEvent(@NonNull final Object event) {
        ...
        for (final SpyEventHandler handler : _spyEventHandlers) {
            try {
                ...
                final String message = ReplicantUtil.safeGetString(
                        () -> "Exception when notifying spy handler '" + handler + "' of '" + event + "' event.");
