...
public class EntityRepositoryDebugger
{
  ...
  public void outputSubscription( final EntityRepository repository,
                                  ...
                                  final Class<?>[] types )
  {
    final ChannelAddress descriptor = new ChannelAddress( graph );
  ...
  {
    final List<Class<?>> types = repository.getTypes();
    for ( final Class<?> type : types )
    {
  ...
  {
    final List<?> entityIDs = repository.findAllIDs( type );
    outputTypeHeader( type, entityIDs.size() );
    Collections.sort( (List) entityIDs );
