...
* Add `SystemSchema.getInwardChannelLinks(int)`, `SystemSchema.getOutwardChannelLinks(int)` and `ChannelSchema.getOutwardChannelLinks()` helper methods. These are not used at runtime but are primarily intended for usage in supporting tooling and testing infrastructure
