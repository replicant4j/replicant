...
public class ReplicantEntityChangeListener {
    ...
    private ReplicantChangeRecorder getRecorder() {
        if (null == _recorder) {
            ...
            final var bean = beanManager
                    .getBeans(ReplicantChangeRecorder.class)
                    .iterator()
                    .next();
            final var creationalContext = beanManager.createCreationalContext(bean);
            _recorder = (ReplicantChangeRecorder)
                    beanManager.getReference(bean, ReplicantChangeRecorder.class, creationalContext);
        }
        return _recorder;
    }
}
