...
public class ChangeUtilTest
{
  ...
  public void basicOperation()
  {
    ...
    final List<Change> changes =
      ChangeUtil.toChanges( Arrays.asList( message1, message2 ), new ChannelDescriptor( 1, 22 ) );

    assertEquals( changes.size(), 2 );

    assertEquals( changes.get( 0 ).getEntityMessage(), message1 );
    assertEquals( changes.get( 0 ).getChannels().get( 1 ), 22 );
    assertEquals( changes.get( 1 ).getEntityMessage(), message2 );
    assertEquals( changes.get( 1 ).getChannels().get( 1 ), 22 );
  }
}
